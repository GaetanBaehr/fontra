...
export class PointerTool extends BaseTool {
  ...
  async handleDrag(eventStream, initialEvent) {
    if (initialEvent[commandKeyProperty] && this.sceneSettings.selectedGlyph?.isEditing) {
      await this.handleContourSelect(eventStream, initialEvent);
      return;
    }
  ...
  async handleContourSelect(eventStream, initialEvent) {
  ...
  // Toujours partir de la sélection courante pour détecter si déjà sélectionné
  const currentSelection = new Set(this.sceneController.selection);

  // La sélection active repart de zéro si pas de shift
  let activeSelection = initialEvent.shiftKey
  ...
  // Click initial — tester contre currentSelection pour savoir si déjà sélectionné
  const initialLocal = getLocalPoint(initialEvent);
  const initialHit = pathHitTester.findNearest(initialLocal);
  const wasFullySelected = initialHit
    ? isContourFullySelected(initialHit.contourIndex, currentSelection)
    : false;

  // Ajoute le contour à activeSelection sans allowDeselect
  if (initialHit) {
    if (wasFullySelected) {
      // Désélectionne au click simple
      removeContourFromSelection(initialHit.contourIndex, activeSelection);
    } else {
      addContourToSelection(initialHit.contourIndex, activeSelection);
    }
  }

  const hitPoint = initialHit ? getHitPoint(initialHit) ?? initialLocal : null;
  if (hitPoint) {
  ...
  // Accumule les contours touchés pendant le drag
  let dragSelection = new Set(activeSelection);

  // Vérifie si c'est un vrai drag ou juste un click
  const isDrag = await shouldInitiateDrag(eventStream, initialEvent);

  if (!isDrag) {
    // Click simple : on garde la désélection, rien à faire
    selectContourLineState = null;
    this.canvasController.requestUpdate();
    return;
  }

  // C'est un drag : si on avait désélectionné, annuler
  if (wasFullySelected && initialHit) {
    addContourToSelection(initialHit.contourIndex, dragSelection);
    this.sceneController.selection = new Set(dragSelection);
    this.canvasController.requestUpdate();
  }

  for await (const event of eventStream) {
    if (!event[commandKeyProperty]) break;
    ...
    this.sceneController.selection = event.shiftKey
